...
3. Describe the uPA distribution graphically.
    ...
    ```{r upa-graphic-distribution, fig.cap='Figure 1: uPA distribution. Logarithmic scale due to one extreme outlier.'}
    breast_cancer |>
      ggplot(aes(x = upa)) +
        geom_histogram() +
        scale_x_log10() +
        labs(
          x = 'uPA (log10(ng/ml))',
          y = 'Count',
          title = 'Histogram of uPA level distribution',
          caption = 'uPA; urokinasse plasminogen activator.'
        )
    ```
...
4. Describe time to first distant metastasis graphically and estimate the fraction, in the underlying population, free from distant metastases five years after diagnosis of primary breast cancer.
    ...
    ```{r km-plot, fig.cap='Figure 2: Kaplan-Meier plot of time until distant metastases.'}
    survfit(Surv(ddfs, distmet) ~ 1, data = breast_cancer) |>
      ggsurvplot(
        legend = 'none',
