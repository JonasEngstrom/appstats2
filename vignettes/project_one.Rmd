...
6. Age is often a confounder in non-randomized studies. Evaluate if the effect of uPA is age dependent by adjusting for age in a multivariable Cox-model.
    ...
    cox_reg_age <- coxph(Surv(ddfs, distmet) ~ upa + age, data = breast_cancer |> mutate(upa = upa * 10))
    summary(cox_reg_age)
    ...
    cox_reg_age_groups <- coxph(Surv(ddfs, distmet) ~ upa + age_group, data = age_groups |> mutate(upa = upa * 10))
